...
Path("gpt2_random/datasets/alice").mkdir(parents=True, exist_ok=True)
Path("gpt2_random/datasets/AG_NEWS").mkdir(parents=True, exist_ok=True)
torch.manual_seed(0)
...
layers = [6]
torch.save(torch.Tensor(layers).long(), f"gpt2_random/datasets/layers.torch")

for b in layers:
    # choice of the self-attention module
    block = model.h[b].attn
    torch.save(block, f"gpt2_random/datasets/block_{b}.torch")
...
torch.save(torch.Tensor(ns).long(), "gpt2_random/datasets/AG_NEWS/ns.torch")
...
for n in ns:
    ...
    for i, b in enumerate(layers):
        ...
        torch.save(dataset_in, f"gpt2_random/datasets/AG_NEWS/dataset_{b}_n={n}.torch")
...
ns = range(2, 100, 2)  # choice of sequence lengths
torch.save(torch.Tensor(ns).long(), f"gpt2_random/datasets/alice/ns.torch")
...
for n in ns:
    ...
    for i, b in enumerate(layers):
        dataset_in = activations[i]
        torch.save(dataset_in, f"gpt2_random/datasets/alice/dataset_{b}_n={n}.torch")
