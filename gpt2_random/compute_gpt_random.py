...
def loc_lipschitz(attention, Y, threshold=1e-3, max_iter=1000, d=768):
...
for b in layers:
    block = torch.load(f"gpt2_random/datasets/block_{b}.torch")
    ...
    for n in ns:
        ...
        means = []
        lipschitz_csts = []
        dataset_in = torch.load(f"gpt2_random/datasets/alice/dataset_{b}_n={n}.torch")
    ...
    torch.save(
        torch.Tensor(np.array(to_plot)), f"gpt2_random/datasets/alice/to_plot_{b}.torch"
    )
...
for b in layers:
    ...
    for n in ns:
        ...
        means = []
        lipschitz_csts = []
        dataset_in = torch.load(f"gpt2_random/datasets/AG_NEWS/dataset_{b}_n={n}.torch")
    ...
    torch.save(
        ...
        f"gpt2_random/datasets/AG_NEWS/to_plot_{b}.torch",
    )
