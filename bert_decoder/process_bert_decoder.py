...
Path("bert_decoder/datasets/alice").mkdir(parents=True, exist_ok=True)
Path("bert_decoder/datasets/AG_NEWS").mkdir(parents=True, exist_ok=True)
...
layers = [6]
torch.save(torch.Tensor(layers).long(), f"bert_decoder/datasets/layers.torch")

for b in layers:
    # choice of the self-attention module
    block = model.encoder.layer[b]
    torch.save(block, f"bert_decoder/datasets/block_{b}.torch")
...
torch.save(torch.Tensor(ns).long(), "bert_decoder/datasets/AG_NEWS/ns.torch")
...
for n in ns:
    ...
    for i, b in enumerate(layers):
        ...
        torch.save(dataset_in, f"bert_decoder/datasets/AG_NEWS/dataset_{b}_n={n}.torch")
...
ns = range(2, 100, 2)  # choice of sequence lengths
torch.save(torch.Tensor(ns).long(), f"bert_decoder/datasets/alice/ns.torch")
...
for n in ns:
    ...
    for i, b in enumerate(layers):
        dataset_in = activations[i]
        torch.save(dataset_in, f"bert_decoder/datasets/alice/dataset_{b}_n={n}.torch")
