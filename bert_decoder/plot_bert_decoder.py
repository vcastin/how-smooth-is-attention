...
Path("bert_decoder/figures").mkdir(parents=True, exist_ok=True)
...
for k, b in enumerate(layers):
    ...
    to_plot = torch.load(f"bert_decoder/datasets/alice/to_plot_{b}.torch")
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(f"bert_decoder/figures/fig_radius_alice_{b}.pdf")
    ...
    ax.plot(
        x_axis, x_axis**0.25 * 6.44, label=r"$cn^{1/4}$", c="darkblue", linewidth=0.9
    )
    ...
    plt.tight_layout()
    plt.grid()
    plt.savefig(f"bert_decoder/figures/fig_n_alice_{b}.pdf")
...
for k, b in enumerate(layers):
    ...
    to_plot = torch.load(f"bert_decoder/datasets/AG_NEWS/to_plot_{b}.torch")
    ...
    plt.tight_layout()
    plt.grid()
    plt.savefig(f"bert_decoder/figures/fig_radius_NEWS_{b}.pdf")
    ...
    plt.tight_layout()
    plt.grid()
    plt.savefig(f"bert_decoder/figures/fig_n_NEWS_{b}.pdf")
