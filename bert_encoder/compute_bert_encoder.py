...
for b in layers:
    block = torch.load(f"bert_encoder/datasets/block_{b}.torch")
    ...
    for n in ns:
        ...
        means = []
        lipschitz_csts = []
        dataset_in = torch.load(f"bert_encoder/datasets/alice/dataset_{b}_n={n}.torch")
    ...
    torch.save(
        ...
        f"bert_encoder/datasets/alice/to_plot_{b}.torch",
    )
...
for b in layers:
    ...
    for n in ns:
        ...
        dataset_in = torch.load(
            f"bert_encoder/datasets/AG_NEWS/dataset_{b}_n={n}.torch"
        )
    ...
    torch.save(
        torch.Tensor(np.array(to_plot)),
        f"bert_encoder/datasets/AG_NEWS/to_plot_{b}.torch",
    )
