...
def outs(block, input: torch.Tensor):
    ...
    x = block(input)  # non residual unnormalized attention, 12 attention heads
    return x[0]  # shape = (batch_size, n_tokens, 768)


def loc_lipschitz(attention, Y, threshold=5e-4, max_iter=500, d=768):
...
for b in layers:
    block = torch.load(f"gpt_2/datasets/block_{b}.torch")
    ...
    for n in ns:
        ...
        means = []
        lipschitz_csts = []
        dataset_in = torch.load(f"gpt_2/datasets/alice/dataset_{b}_n={n}.torch")
    ...
    torch.save(
        torch.Tensor(np.array(to_plot)), f"gpt_2/datasets/alice/to_plot_{b}.torch"
    )


#### AG_NEWS

ns = torch.load("gpt_2/datasets/AG_NEWS/ns.torch")
...
for b in layers:
    ...
    for n in ns:
        ...
        means = []
        lipschitz_csts = []
        dataset_in = torch.load(f"gpt_2/datasets/AG_NEWS/dataset_{b}_n={n}.torch")
    ...
    torch.save(
        torch.Tensor(np.array(to_plot)), f"gpt_2/datasets/AG_NEWS/to_plot_{b}.torch"
    )
